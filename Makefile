...
LOCAL_MK       ?= $(PROJECT_DIR)/retrolx.mk
EXTRA_PKGS     ?=
...
TARGETS := $(sort $(shell find $(PROJECT_DIR)/configs/ -name 'r*' | sed -n 's/.*\/retrolx-\(.*\)_defconfig/\1/p'))
...
%-menuconfig: %-config ccache-dir dl-dir
	 make $(MAKE_OPTS) O=$(OUTPUT_DIR)/$* BR2_EXTERNAL=$(PROJECT_DIR) -C $(PROJECT_DIR)/buildroot menuconfig
